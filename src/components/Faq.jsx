      ...
      <div className="container-x grid gap-14 lg:grid-cols-[1fr_1.4fr]">
        <Reveal>
          ...
          <div className="card mt-8 max-w-sm p-6">
            ...
            <p className="mt-1 text-sm text-night-950/50">
              Every email gets a human answer — usually the same day.
            </p>
            <a
              href={BRAND.emailUrl}
              className="mt-3 inline-block text-sm font-semibold text-accent-600 transition hover:text-accent-500"
            >
              {BRAND.email}
            </a>
            <a href={BRAND.emailUrl} className="btn-ghost mt-5 w-full">
            ...
            {FAQS.map((f, i) => (
              ...
              <div className="border-b border-ice-200 last:border-0">
                <button
                  ...
                  className="flex w-full items-center justify-between gap-4 px-6 py-5 text-left transition hover:bg-ice-50"
                  aria-expanded={open === i}
                >
                  <span className="font-semibold">{f.q}</span>
                  <span
                    className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-ice-100 text-night-950 transition-transform duration-300 ${
                      open === i ? "rotate-45" : ""
                    }`}
                  >
                ...
                >
                  <div className="overflow-hidden">
                    <p className="px-6 pb-6 text-sm text-night-950/60">{f.a}</p>
                  </div>
                </div>
              </div>
              </Reveal>
