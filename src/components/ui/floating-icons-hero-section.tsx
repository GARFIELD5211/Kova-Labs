...
// --- Stylized Brand & Ecosystem SVG Components ---
...
export const IconApple = (props: React.SVGProps<SVGSVGElement>) => (
  <svg {...props} viewBox="0 0 24 24" fill="currentColor" className="text-foreground/90" xmlns="http://www.w3.org/2000/svg">
    <path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.81-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M15.97 6.37c.61-.75 1.04-1.8 0.92-2.87-.9.04-2.02.6-2.67 1.34-.56.65-1.08 1.73-.95 2.76 1.02.08 2.06-.48 2.7-1.23z"/>
  </svg>
);
...
export const IconFigma = (props: React.SVGProps<SVGSVGElement>) => (
  <svg {...props} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <path d="M8 16h4v3.5A3.5 3.5 0 0 1 8 16z" fill="#0ACF83"/>
  </svg>
);
...
export const IconVercel = (props: React.SVGProps<SVGSVGElement>) => (
  <svg {...props} viewBox="0 0 24 24" fill="currentColor" className="text-foreground/95" xmlns="http://www.w3.org/2000/svg">
...
// Define default 16 unique icons with coordinates well-spaced around borders
export const demoIcons: IconProps[] = [
  { id: 1, icon: IconGoogle, className: 'top-[16%] left-[3%] sm:left-[6%]' },
  { id: 2, icon: IconApple, className: 'top-[16%] right-[3%] sm:right-[7%]' },
  { id: 3, icon: IconMicrosoft, className: 'bottom-[12%] left-[4%] sm:left-[7%]' },
  { id: 4, icon: IconFigma, className: 'bottom-[12%] right-[4%] sm:right-[8%]' },
  { id: 5, icon: IconGitHub, className: 'top-[12%] left-[22%] sm:left-[26%]' },
  { id: 6, icon: IconSlack, className: 'top-[12%] right-[22%] sm:right-[26%]' },
  { id: 7, icon: IconVercel, className: 'bottom-[8%] left-[22%] sm:left-[20%]' },
  { id: 8, icon: IconStripe, className: 'hidden sm:block top-[44%] left-[3%] sm:left-[5%]' },
  { id: 9, icon: IconDiscord, className: 'hidden sm:block top-[70%] right-[12%] sm:right-[15%]' },
  { id: 10, icon: IconX, className: 'bottom-[8%] right-[22%] sm:right-[26%]' },
  { id: 11, icon: IconNotion, className: 'hidden sm:block top-[46%] right-[4%] sm:right-[6%]' },
  { id: 12, icon: IconSpotify, className: 'top-[66%] left-[4%] sm:left-[9%]' },
  { id: 13, icon: IconDropbox, className: 'hidden sm:block top-[14%] left-[48%]' },
  { id: 14, icon: IconTwitch, className: 'hidden sm:block bottom-[8%] left-[48%]' },
  { id: 15, icon: IconLinear, className: 'hidden sm:block top-[28%] right-[12%] sm:right-[14%]' },
  { id: 16, icon: IconYouTube, className: 'hidden sm:block bottom-[10%] left-[32%] sm:left-[34%]' },
];

// A single icon component with its own motion logic and repulsion
const Icon = ({
  mouseX,
  mouseY,
  iconData,
  ...
  return (
    <motion.div
      ...
      className={cn('absolute pointer-events-auto select-none', iconData.className)}
    >
      {/* Inner wrapper for continuous floating animation */}
      <motion.div
        className="flex items-center justify-center w-10 h-10 sm:w-16 sm:h-16 md:w-20 md:h-20 p-2 sm:p-3 rounded-xl sm:rounded-2xl md:rounded-3xl shadow-[0_12px_28px_rgba(0,0,0,0.45)] bg-card/85 backdrop-blur-xl border border-white/10 hover:border-accent/40 transition-colors"
        animate={{
          y: [0, -8, 0, 8, 0],
          x: [0, 6, 0, -6, 0],
          rotate: [0, 5, 0, -5, 0],
      ...
      >
        <iconData.icon className="w-5 h-5 sm:w-8 sm:h-8 md:w-10 md:h-10 text-foreground transition-transform duration-300 hover:scale-115" />
      </motion.div>
    </motion.div>
  );
};
...
>(({ className, title, subtitle, ctaText, ctaHref, icons = demoIcons, children, ...props }, ref) => {
  ...
  return (
    <section
      ...
      className={cn(
        'relative w-full min-h-[100svh] flex items-center justify-center overflow-hidden bg-background text-foreground',
        className
      )}
      {...props}
    >
